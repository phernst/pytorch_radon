...
class Stackgram(nn.Module):
    def __init__(self, out_size, theta=None, circle=True, mode='nearest',
        ...
        self.in_size = in_size = out_size \
            if circle else int((SQRT2*out_size).ceil())
        self.dtype = dtype
    ...
    def forward(self, x):
        stackgram = torch.zeros(
            x.shape[0], len(self.theta), self.in_size, self.in_size,
            device=x.device,
            dtype=self.dtype,
        )
...
class IStackgram(nn.Module):
    ...
    def forward(self, x):
        sinogram = torch.zeros(
            x.shape[0], 1, self.in_size, len(self.theta),
            device=x.device,
            dtype=self.dtype,
        )
